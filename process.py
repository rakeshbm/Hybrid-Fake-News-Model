...
#rules for cleaning punctuation
...
def process(X):
    ...
    global sel_punc, sel_punc2, final_cap, total_exclam, total_question
    X_processed = []
...
def normalize_word(w):
    return nltk.WordNetLemmatizer().lemmatize(w).lower()
...
def main():
    ...
    global news, total_exclam, total_question, final_cap
    ...
    #preprocess and tokenize the read data
    X_processed = process(X)
    
    #compute length of each news article
    find_length()
    
    #compute sentiment polarity and subjectivity for each news article text 
    ...
    #compute sentiment polarity and subjectivity for each news article title
    ...
    #perform readability test on the data
    news['fog_index'] = [textstat.gunning_fog(text) for text in news.text]
    
    #compute afinn sentiment scores for the data
    afinn = Afinn(emoticons=True)
    news['afinn'] = [afinn.score(text) for text in news.text]
    ...
    for i in range(len(corpus)):
        ...
        topic0.append(prob[0][1])
        topic1.append(prob[1][1])
        topic2.append(prob[2][1])
        topic3.append(prob[3][1])
        
    lda_prob = pd.DataFrame({"T0":topic0, "T1":topic1,"T2":topic2, "T3":topic3})
    
    news = news.join(lda_prob)
    ...
    X_stance = generate_features(news,"new")

    news = news.join(pd.DataFrame(X_stance))
    
    news.to_csv("processed_data.csv", encoding="utf-8")
